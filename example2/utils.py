...
def generate_kube_config_yaml(eks_cluster) -> pulumi.Output:
    kubeconfig = pulumi.Output.all(eks_cluster.endpoint, eks_cluster.certificate_authority.apply(lambda v: v.data), eks_cluster.name).apply(lambda args: yaml.dump({
        "apiVersion": "v1",
        "clusters": [{
            "cluster": {
                "server": args[0],
